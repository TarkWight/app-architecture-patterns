...
application::dto::PdfDocumentModel ExportPdfUseCase::buildDocument() const {
    ...
    const auto &lines = session.protocol.testProtocol.lines;

    application::dto::PdfDocumentModel document{};
    document.title =
        valueOrDefault(session.protocol.testProtocol.title, "Протокол испытаний беспилотной авиационной системы");
    ...
    document.testMode = domain::testModeTitle(session.protocol.testProtocol.testMode);
    document.testProgramTitle = domain::testProgramTitle(session.protocol.testProtocol.testProgram);
    document.testProgramShortTitle = domain::testProgramShortTitle(session.protocol.testProtocol.testProgram);
    ...
    document.droneParameters = session.protocol.testProtocol.droneParameters;
    ...
    row.windFormula = valueOrDefault(session.control.functionExpression.value, "0");
    row.estimatedDurationMinutes = std::to_string(session.protocol.estimatedTestDuration.value());
    row.testDurationMinutes = std::to_string(session.execution.activeTestDuration.value());
    row.result = valueOrDefault(lines[6], "_________");
    document.tableRows.push_back(std::move(row));

    document.telemetryPlot = session.telemetry.telemetryPlot;
    document.telemetryPlotCaption = "Рисунок 2 - Изменение положения стенда во время теста.";

    document.controlPlot = session.control.controlPlot;
